 ...
 */
@Config
@Autonomous(name="AAutoLeftBlueBackDropOld", group = "drive")
@Disabled
public class XAutoLeftBlueBackDropOld extends LinearOpMode {
    /*
    ...
    */
    ...
    XAutoLeftBlueBackDropOld.State currentState = XAutoLeftBlueBackDropOld.State.STATE_INITIAL;
    int ver = 1;
...
// Position 2
        ...
        traj_STATE_LEFT_POS2_STEP2 = drive.trajectoryBuilder(traj_STATE_LEFT_POS2_STEP1.end())
                .lineToLinearHeading(new Pose2d(-28.5,-41, Math.toRadians(90)))
                .build();
...
// Position 3
        ...
        while(!isStarted() && !isStopRequested()){
            ...
            /*
            try {
                if (tfod.getRecognitions() != null && !tfod.getRecognitions().isEmpty()) {

                       Recognition recognition = tfod.getFreshRecognitions().get(0);
                       x = (recognition.getLeft() + recognition.getRight()) / 2;
                       y = (recognition.getTop() + recognition.getBottom()) / 2;
                ...
                //tfodEP = 1;
                //telemetry.addData("- Exception Position =>", tfodEP);
            }
             */
            sleep(20);
            telemetry.update();
